...
class Objtrack:
     def __init__(self):
        ...
        self.vehicle = connect('/dev/ttyUSB0,57600', wait_ready=True)
        self.frame=None
        self.webframe=None
        ...
        if circles is not None:
           ...
           if len(circles) > 0:
               ...
               if kp_max is not None and kp_max[2]>20 and kp_max[2]<200:
                  ...
                  cv2.circle(frame,(int(kp_max[0]),int(kp_max[1])),int(kp_max[2]),(255,255,255),1)
     ...
     def object_find(self,frame):
        ...
        filter_low =np.array([138,13,179]) #for red circle
        filter_high=np.array([163,255,255])
     ...
     def run(self):
       ...
       # print "RC6MIN=",RC6MIN,",RC6MAX=",RC6MAX,",RC6MID=",RC6MID
        ...
        try:
            ...
            video_name='IOPD_%s.avi' % (time.time())
            ...
            log_gname='IOPD_%s.txt' % (time.time())
            ...
            while(True):
              ...
              #self.object_found, xpos, ypos, size = balloon_finder.analyse_frame(newframe)
              ...
              if RC6VAL<RC6MID:#vision-controlled
                 ...
                 if self.vehicle.mode.name=="GUIDED":#visual-guided
                    ...
                    if self.object_found: 
                       ...
                       yerror=deltay
                       yderivative = (yerror - yprevious_error)/dt

                       upx=deltay*gain#+Kd*yderivative
                       #get the x-velocity control 
                       
                       yprevious_error=yerror
                       #print "dt:",dt

                       xerror=deltax
                       xderivative=(xerror-xprevious_error)/dt
                       upy=deltax*gain#+Kd*xderivative
                       ...
                       gf.write("%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\n"%(time.time(),dt,yaw_origin,xpos,ypos,xderivative,upx,upy,gain,size,roll_origin,pitch_origin,RC6VAL))
                       gf.close()
